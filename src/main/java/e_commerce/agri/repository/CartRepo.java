...
public interface CartRepo extends JpaRepository<Cart, Long> {
    ...
    @Query("SELECT COUNT(c) > 0 FROM Cart c WHERE c.customer.emailId = :emailId AND c.product.productId = :productId")
    boolean isProductInCart(@Param("emailId") String emailId, @Param("productId") long productId);
}
